...
사용자 → 프론트 서버  : 초기 접속 요청
프론트 서버          : JS 실행
프론트 서버 → 브라우저 : 완성된 HTML 반환
브라우저 → 사용자     : 화면 렌더링 (FCP) — 아직 상호작용 불가
서버 → 브라우저      : JS 번들 반환
브라우저            : 수화(Hydration) 완료
브라우저 → 사용자     : 상호작용 가능 (TTI)
```
...
[빌드 타임]
프론트 서버 → 백서버  : API 요청
백서버 → 프론트 서버  : 데이터 응답 → HTML 생성

[런타임]
사용자 → 프론트 서버  : 초기 접속 요청
프론트 서버 → 브라우저 : 미리 만들어둔 HTML 반환 (매우 빠름)
```
